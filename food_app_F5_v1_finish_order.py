    ...
    def show_thank(self):
        try:
            if self.name_e_bill.get() == "": 
                self.thanks.configure(text = "Please enter Name") 
                self.name_e_bill.focus() 

            elif self.name_e_bill.get().isalpha() == False: 
                self.thanks.configure(text = "Pleasae enter text only") 
                self.name_e_bill.delete(0, END) 
                self.name_e_bill.focus()
            
            elif self.phone_entry.get =="":
                self.thanks.config(text = "Please enter Phone Number")
                self.phone_entry.focus()
            
            elif self.phone_entry.get().isalpha() == True:
                self.thanks.configure(text = "Pleasae enter number only") 
                self.phone_entry.delete(0, END) 
                self.phone_entry.focus()


            elif self.address_entry.get()== "":
                self.thanks.configure(text = "Please enter Address") 
                self.name_e_bill.focus() 
